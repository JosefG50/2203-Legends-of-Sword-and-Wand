...
public class ItemConsumeUseCase {
    ...
    public ItemConsumeResponseDTO execute(ItemType itemType, int heroIndex) {
        ...
        if (itemType.canRevive() && hero.getCurHp() == 0) {
            int heal = hero.getMaxHp();
            hero.gainCurHp(heal);
            hpRestored = heal;
            revived = true;
        } else {
            int heal = itemType.getHpHeal();

            int actualHeal = Math.min(heal, hero.getMaxHp() - hero.getCurHp());

            hero.gainCurHp(actualHeal);

            hpRestored = actualHeal;
        }

        int manaHeal = itemType.getManaHeal();
        hero.gainCurMana(manaHeal);
